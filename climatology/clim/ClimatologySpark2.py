...
AveragingConfigs = {'pixelMean': PixelMeanConfig, 'gaussInterp': GaussInterpConfig1a, 'spatialFilter': SpatialFilterConfig1}
...
def climByAveragingPeriods(urls,              # list of (daily) granule URLs for a long time period (e.g. 15 years), *SORTED* by DOY
                    ...
                    outHdfsPath,              # HDFS path to write outputs to (i.e. netCDF file containing stats and optional contour plot)
                    ...
                    sparkConfig='mesos,8,16', # Map across time periods of N-days for concurrent work, executed by:
...
Optionally generates a contour plot of each N-day climatology for verification.
    ...
    with Timer("Parallel Stats"):
        if sparkConfig.startswith('dask'):
            outputFiles = parallelStatsDaskSimple(urlSplits, datasetInfo, nEpochs, variable, mask, coordinates, reader, outHdfsPath,
                                                  averagingConfig, sparkConfig, accumulators)
        else:
            outputFiles = parallelStatsSparkSimple(urlSplits, datasetInfo, nEpochs, variable, mask, coordinates, reader, outHdfsPath,
                                                  averagingConfig, sparkConfig, accumulators)
    return outputFiles


def parallelStatsSparkSimple(urlSplits, ds, nEpochs, variable, mask, coordinates, reader, outHdfsPath, averagingConfig, sparkConfig,
    ...
    with Timer("Write out climatology and optionally plot"):              # write stats to netCDF file
        outputFiles = stats.map(lambda s: writeAndPlot(s, ds, variable, coordinates, nEpochs, averagingConfig, outHdfsPath, plot=False))  \
                           .collect()
    return outputFiles


def parallelStatsSpark(urlSplits, ds, nEpochs, variable, mask, coordinates, reader, outHdfsPath, averagingConfig, sparkConfig,
                       accumulators=['count', 'mean', 'M2', 'min', 'max']):
    '''Compute N-day climatology statistics in parallel using PySpark or pysparkling.'''
    with Timer("Configure Spark"):
        sparkContext, numExecutors, numPartitions = configureSpark(sparkConfig, appName='parallelClimStats')
...
def parallelStatsPipeline(urls, ds, nEpochs, variable, mask, coordinates, reader, averagingConfig, outHdfsPath, accumulators=['count', 'mean', 'M2', 'min', 'max']):
    '''Hide entire pipeline in a single function.'''
    outputFile = writeAndPlot( statsFromAccumulators( accumulate(urls, variable, mask, coordinates, reader, accumulators)),
                               ds, variable, coordinates, nEpochs, averagingConfig, outHdfsPath, plot=False)
    return outputFile


def parallelStatsDaskSimple(urlSplits, ds, nEpochs, variable, mask, coordinates, reader, outHdfsPath, averagingConfig, sparkConfig,
                            accumulators=['count', 'mean', 'M2', 'min', 'max']):
    '''Compute N-day climatology statistics in parallel using PySpark or pysparkling.'''
    ...
    start = time.time()    
    futures = client.map(lambda urls: parallelStatsPipeline(urls, ds, nEpochs, variable, mask, coordinates, reader, averagingConfig, outHdfsPath, accumulators), urlSplits)
...
If the URL is remote or HDFS, first retrieve the file into a cache directory.
    ...
    try:
        ...
        var, fh = getVariables(path, variables, arrayOnly=True, set_auto_mask=True)   # return dict of variable objects by name                                                
...
def accumulate(urls, variable, maskVar, coordinates, reader=readAndMask, accumulators=['count', 'mean', 'M2', 'min', 'max'], cachePath=CachePath, hdfsPath=None):
...
#        dout.setncattr(a, din.getncattr(a))
    ...
    try:
        ...
        if 'lat' in coorindatesFromFile.lower() and 'lon' in coordinatesFromFile.lower():
            ...
            if coordinates[0].lower() == 'time':   coordinates = coordinates[1:]    # discard trivial time dimension for CF-style files
        else:
...
def computeClimatology(datasetName, nEpochs, nWindow, averager, outHdfsPath, sparkConfig):
    ...
    if averager not in AveragingFunctions:
        print >>sys.stderr, 'computeClimatology: Error, averager %s must be in set %s' % (averager, str(AveragingFunctions.keys()))
        sys.exit(1)
    try:
    ...
    with Timer("climByAveragingPeriods"):
        results = climByAveragingPeriods(urls, ds, nEpochs, nWindow, ds.Variable, ds.Mask, ds.Coordinates,
                                         readAndMask, outHdfsPath, ds.split, averager=averager,
                                         averagingConfig=AveragingConfigs[averager], sparkConfig=sparkConfig)
    return results
