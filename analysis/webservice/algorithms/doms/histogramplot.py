...
def renderAsync(x, primary, secondary, parameter):
    manager = Manager()
    d = manager.dict()
    p = Process(target=render, args=(d, x, primary, secondary, parameter))
    p.start()
    p.join()
    return d['plot']


def createHistogramPlot(id, parameter):
    ...
    plot = renderAsync(x, primary, secondary, parameter)
