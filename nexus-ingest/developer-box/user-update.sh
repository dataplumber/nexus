...
echo "NOTICE: You will be prompted for a GitHub OAuth token in order to update repositories. A token can be generated by going to https://github.com/settings/tokens"
sleep 2s
read -p "Please enter a Github OAuth token: " token

pushd nexus
git pull https://$token:x-oauth-basic@github.com/dataplumber/nexus.git
popd
