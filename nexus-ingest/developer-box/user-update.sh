...
echo "NOTICE: You will be prompted for a GitHub OAuth token in order to update repositories. A token can be generated by going to https://github.jpl.nasa.gov/settings/tokens"
sleep 2s
read -p "Please enter a JPL Github OAuth token: " token

pushd nexus
git pull https://$token:x-oauth-basic@github.jpl.nasa.gov/thuang/nexus.git
popd
